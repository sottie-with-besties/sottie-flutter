import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:sottie_flutter/core/constant/custom_colors.dart';
import 'package:sottie_flutter/ui/post/controller/post_options_setting.dart';
import 'package:sottie_flutter/ui/search/controller/date_time_reset.dart';
import 'package:time_range_picker/time_range_picker.dart';

class TimeRangeOption extends ConsumerStatefulWidget {
  const TimeRangeOption({super.key});

  @override
  ConsumerState<TimeRangeOption> createState() => _TimeRangeClassState();
}

class _TimeRangeClassState extends ConsumerState<TimeRangeOption> {
  String timeString = '시간 범위 선택';

  void makeTimeString() {
    if (postOptionsSetting.timeStart == null ||
        postOptionsSetting.timeEnd == null) {
      timeString = "시간 범위 선택";
      return;
    }

    int startHour = postOptionsSetting.timeStart!.hour;
    String startTemp = startHour < 12 ? "AM" : "PM";

    startTemp == "PM"
        ? startHour > 12
            ? startHour -= 12
            : null
        : null;

    int endHour = postOptionsSetting.timeEnd!.hour;
    String endTemp = endHour < 12 ? "AM" : "PM";

    endTemp == "PM"
        ? endHour > 12
            ? endHour -= 12
            : null
        : null;

    timeString =
        "$startTemp $startHour시 ${postOptionsSetting.timeStart!.minute}분 ~ $endTemp $endHour시 ${postOptionsSetting.timeEnd!.minute}분";
  }

  @override
  Widget build(BuildContext context) {
    // 날짜 및 시간 초기화 버튼 눌리는거 인지 하는 용도
    ref.watch(dateTimeResetProvider);
    makeTimeString();

    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        OutlinedButton(
          style: OutlinedButton.styleFrom(minimumSize: const Size(0, 50)),
          onPressed: () async {
            TimeRange? tempTime = await showTimeRangePicker(
              context: context,
              start:
                  postOptionsSetting.timeStart ??
                  const TimeOfDay(hour: 0, minute: 0),
              end:
                  postOptionsSetting.timeEnd ??
                  const TimeOfDay(hour: 0, minute: 0),
              barrierDismissible: false,
              interval: const Duration(minutes: 30),
              strokeWidth: 24,
              ticks: 24,
              ticksWidth: 3,
              ticksColor: Colors.black38,
              strokeColor: Colors.deepPurple.shade200,
              snap: true,
              labels:
                  [
                    "12 am",
                    "3 am",
                    "6 am",
                    "9 am",
                    "12 pm",
                    "3 pm",
                    "6 pm",
                    "9 pm",
                  ].asMap().entries.map((e) {
                    return ClockLabel.fromIndex(
                      idx: e.key,
                      length: 8,
                      text: e.value,
                    );
                  }).toList(),
              labelOffset: -30,
              labelStyle: const TextStyle(
                fontSize: 16,
                color: AppColors.greyColor,
                fontWeight: FontWeight.bold,
              ),
            );

            if (tempTime == null) return;

            postOptionsSetting.timeStart = tempTime.startTime;
            postOptionsSetting.timeEnd = tempTime.endTime;

            makeTimeString();
            setState(() {});
          },
          child: Text(timeString, style: const TextStyle(color: Colors.black)),
        ),
      ],
    );
  }
}
