...
class _EventsList extends StatefulWidget {
  ...
  // 서버로부터 받은 이벤트 리스트
  final InChatEventListModel entity;
...
class _EventsListState extends State<_EventsList> with WidgetsBindingObserver {
  ...
  final List<List<InChatEventModel>> _groupEvents = [];
  ...
  Widget _renderEvent(List<InChatEventModel> eventGroup) {
    ...
    switch (event.inChatEventType) {
        ...
        final userList =
            _users
                .where(
                  (user) =>
                      eventGroup.any(
                        (event) => event.userId == user.id.toString(),
                      ) &&
      ...
      case InChatEventType.CHAT:
        ...
        // 내 메시지인지 확인
        final isMyMessage = event.userId == myId.toString();
  ...
  List<List<InChatEventModel>> _groupConsecutiveMessages(
    List<InChatEventModel> events,
  ) {
    if (events.isEmpty) return [];

    final List<List<InChatEventModel>> groups = [];
    List<InChatEventModel> currentGroup = [events.first];
  ...
  bool _checkConsecutiveChat(
    InChatEventModel currentEvent,
    InChatEventModel previousEvent,
  ) {
  ...
  bool _shouldShowDateDivider(
    List<InChatEventModel> currentGroup,
    List<InChatEventModel> previousGroup,
  ) {
